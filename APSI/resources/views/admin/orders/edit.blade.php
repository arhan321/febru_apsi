    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.orders.update', [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_telepon') ? 'is-invalid' : '' }}" type="text" name="no_telepon" id="no_telepon" value="{{ old('no_telepon', $order->no_telepon) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('alamat') ? 'is-invalid' : '' }}" type="alamat" name="alamat" id="alamat" value="{{ old('alamat', $order->alamat) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}" name="products[]" id="products" multiple required>
                    @foreach($products as $product)
                        <option value="{{ $product->id }}" {{ (in_array($product->id, old('products', $order->products->pluck('id')->toArray())) ? 'selected' : '') }}>{{ $product->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('pengiriman') ? 'is-invalid' : '' }}" name="pengiriman" id="pengiriman">
                    @foreach(App\Models\Order::PENGIRIMAN as $key => $label)
                        <option value="{{ $key }}" {{ old('pengiriman', $order->pengiriman) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}" type="number" name="total" id="total" value="{{ old('total', $order->total) }}" step="0.01" readonly>
                @if($errors->has('total'))
                    <div class="invalid-feedback">
                        {{ $errors->first('total') }}
                    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const pesananQuantities = @json($order->products->pluck('pivot.quantity', 'id'));
